...
\usepackage{makeidx}
% \usepackage{helvet}
\usepackage[liberation]{libertine}
\usepackage{graphicx}
\usepackage{sectsty}
...
\begin{document}
  ...
  \begin{figure}[!h]
    \centering
    %\includegraphics[scale=0.2]{logo/recorte.png}
  ...
  \begin{center}
    ...
    IMPLEMENTAÇÃO DE OPERADORES DE PROCESSAMENTO DE IMAGENS EM OPENCL }
  ...
  O uso de imagens na medicina é muito disseminado, por isso, é de bastante importância
  tanto para o diagnóstico quanto para as pesquisas. A implementação de
  processamento de imagens médicas em tempo real demanda o uso de computadores
  com muitos núcleos e uma grande capacidade de memória.

  A fim de minimizar o tempo de processamento utilizamos a linguagem OpenCL. Porém,
  programar em OpenCL acaba sendo difícil, pois a execução de um \textit{kernel}
  precisa de muitas chamadas á API. Essas chamadas são feitas através de um \textit{wrapper},
  que pode ser gerado automaticamente. Um \textit{wrapper} é uma classe que contém
  a instância de outra classe, mas não expõe diretamente essa instância. Para o nosso
  estudo utilizamos a VisionGL~\cite{visiongl} que fornece a geração de um código
  \textit{wrapper} para CUDA, GLSL e OpenCL.

  O aumento da demanda por maior desempenho computacional, especialmente em
  áreas que requer processamento de grande volume de dados em diversas
  estruturas, levou ao crescente uso de arquiteturas heterogêneas como GPUs e Aceleradores
  diversos. A diversidade de arquiteturas contém desafios para portabilidade de
  ferramentas computacionais. A especificação SYCL \cite{sycl2020} apresenta uma
  camada de abstração de que permite o desenvolvimento de programas para processamento
  heterogêneo em C++ com APIs para trabalhar CPUs, GPUs e FPGAs. Em nosso estudo
  utilizamos Intel oneAPI Data Parallel C++ Compiler \cite{inteldpcpp}, uma
  implementação da especificação SYCL, para analisar a viabilidade para processamento
  de imagens médicas.
  ...
  O objetivo deste estudo é ampliar o suporte ao processamento de imagens
  tridimensionais da biblioteca VisionGL e adicionalmente verificar a
  viabilidade do SYCL para processamento de imagens. Para atingir os objetivos foram
  feitos os seguintes passos:

  \begin{enumerate}
    \item Desenvolver operadores existentes na biblioteca VisionGL em SYCL.

    \item Comparar o desempenho desses operadores com o de outras ferramentas de
      processamento de imagens.

    \item Analisar os resultados e aplicar os operadores na biblioteca VisionGL.
  \end{enumerate}

  Embora o projeto não tenha sido concluído em sua totalidade, este relatório descreve
  as etapas realizadas até a interrupção da pesquisa.
  ...
  VisionGL para adaptação dos conceitos originais para a especificação SYCL, explorando
  sua capacidade de gerenciar computação heterogênea e paralelismo de forma. A
  implementação dos operadores foi realizada utilizando o Intel oneAPI Data
  Parallel C++ Compiler, que é uma implementação da especificação SYCL.

  Escolhemos os seguintes operadores para implementação:
  \begin{itemize}
    \item operador de filtro gaussiano
  ...
  concluída e está disponível no repositório do projeto \cite{visionsycl}.
