...
Para SYCL com AdaptiveCPP e CUDA com Julia foram utilizadas estruturas de dados genéricas para armazenar os dados de entrada e saída ao invés de texturas por serem recursos experimentais não suportados por suas implementações.

\subsection{Implementação dos operadores para SYCL}

Para cada operador, foi criado um \textit{functor} para servir como \textit{kernel} de execução. O gerenciamento de memória foi realizado por meio de USM, que não abstraem a transferência de dados entre CPU e GPU. Para o desenvolvimento do \textit{kernel} para ser executado utilizamos a estrutura descrita na Listagem~\ref{lst:sycl-kernel-struct}.
...
\begin{lstlisting}[caption={Implementação do operador de inversão.}, label={lst:sycl-inversion-kernel}]
...
\begin{lstlisting}[caption={Implementação do operador de escala de cinza.}, label={lst:sycl-grayscale-kernel}]
...
\begin{lstlisting}[caption={Implementação do operador de limiarização.}, label={lst:sycl-threshold-kernel}]
for (size_t c = 0; c < channels; ++c) {
  size_t index = (y * width + x) * channels + c;
...
\begin{lstlisting}[caption={Implementação do operador de erosão.}, label={lst:sycl-erosion-kernel}]
for (size_t c = 0; c < channels; ++c) {
  uint8_t result = 255;
...
\begin{lstlisting}[caption={Implementação do operador de convolução.}, label={lst:sycl-convolution-kernel}]
for (size_t c = 0; c < channels; ++c) {
  float sum = 0.0f;
...
A implementação em CUDA utilizou o pacote CUDA.jl para a linguagem Julia, que fornece uma interface de alto nível para programação CUDA. Para o desenvolvimento do \textit{kernel} para ser executado utilizamos a estrutura descrita na Listagem~\ref{lst:cudajl-kernel-struct}.
...
\begin{lstlisting}[caption={Implementação do operador de inversão.}, label={lst:cudajl-inversion-kernel}]
for c in 1:num_channels
  output[c,x,y] = 1.0f0 - input[c,x,y]
end
\end{lstlisting}

\begin{lstlisting}[caption={Implementação do operador de escala de cinza.}, label={lst:cudajl-grayscale-kernel}]
...
\begin{lstlisting}[caption={Implementação do operador de limiarização.}, label={lst:cudajl-threshold-kernel}]
for c in 1:num_channels
  output[c,x,y] = input[c,x,y] > threshold_value ? 1.0f0 : 0.0f0
end
\end{lstlisting}

\begin{lstlisting}[caption={Implementação do operador de erosão.}, label={lst:cudajl-erosion-kernel}]
...
\begin{lstlisting}[caption={Implementação do operador de convolução.}, label={lst:cudajl-convolution-kernel}]
