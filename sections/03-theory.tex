...
Nas subseções seguintes serão descritos alguns conceitos essenciais para o desenvolvimento de operadores para esta pesquisa.

% Revisar ----------------
\subsubsection{Operações ponto-a-ponto e de janela}

Uma operação é dita ponto-a-ponto quando processa os dados de cada píxel da imagem resultante sem depender de dados vizinhos, já uma operação é dita de janela quando processa os dados de cada píxel da imagem utilizando uma janela (matriz) baseado nos dados de píxeis vizinhos para aplicar alteração no píxel alvo.
