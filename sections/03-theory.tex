...
Para que seja possível produzir programas utilizando GPU, tanto o dispositívo  deve suportar as operações presentes nas especificações modelo expecífico quanto o controlador do sistema operacional deve disponibilizar a API (do inglês, \textit{Application Programming Interface}) para que o desenvolvedor possar fazer uso.

A Khronos Group é a principal responsável pelas especificações atuais de comunicação com GPU, mas existem APIs específicas para dispositívos e sistemas operacionais. Será dado foco ao CUDA (desenvolvido pela NVIDIA), OpenCL e SYCL (desenvolvido pela Khronos Group) nas seções posteriores.
