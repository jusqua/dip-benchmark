...
Produto matricial é comumente utilizado em operações de transformação geométrica, como rotação, escala e translação de imagens que é um ponto de foco para computação gráfica~\cite{gonzalez17,shirley15}. Logo, quando uma operação ser apresentada nesse trabalho será considerado exclusivamente uma operação pontual, a menos que seja explicitamente declarado como uma operação matricial.
...
Nesta seção, serão apresentadas as principais tecnologias e modelos de programação para computação paralela em GPUs que serão postos a prova. Especificamente, abordaremos CUDA, OpenCL e SYCL, que são amplamente utilizados no desenvolvimento de aplicações de alto desempenho.
...
float* memA = sycl::malloc_device<float>(size);
float* memB = sycl::malloc_device<float>(size);
float* memC = sycl::malloc_device<float>(size);
