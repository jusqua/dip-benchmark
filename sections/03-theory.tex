...
Nas subseções seguintes serão descritos alguns conceitos essenciais para o desenvolvimento de operadores para esta pesquisa. Para os exemplos seguintes considere as seguintes imagens (matrizes):
\[
	I_1 =
	\begin{bmatrix}
		a_{11} & a_{12} \\
...
\end{enumerate}
Em consequência, para combinações lineares genéricas tem-se \(L(a\,I_1 + b\,I_2) = a\,L(I_1) + b\,L(I_2)\). Quaisquer operadores que não satisfazem essas propriedades são chamados não-lineares.

\subsubsection{Operações espaciais}
