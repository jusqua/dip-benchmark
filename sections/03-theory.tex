...
Uma operação é dita pontual quando processa os dados de cada pixel da imagem resultante sem depender de dados vizinhos, já uma operação é dita matricial quando processa os dados de cada pixel da imagem utilizando todo o escopo da imagem aplicando transformações lineares~\cite{gonzalez17}.
...
Um operador é dito de janela quando processa uma imagem considerando a vizinhança dos pixels. Um \textit{kernel} representado por uma matriz de pesos aplicada sobre a imagem original para produzir a imagem resultante. Com isso, cada pixel da imagem resultante depende dos valores dos pixels vizinhos na imagem original. Considerando uma máscara \(K\) de tamanho \(m \times n\), a operação de convolução é um exemplo de operador de janela, definida como:
\begin{equation}
...
GPUs passaram a ser usadas para computação de propósito geral (general-purpose graphics processing unit, GPGPU) em aplicações com algoritmos altamente paralelizáveis~\cite{stallings15computer,james23dpcpp}, como processamento de imagens e aprendizados de máquina. Embora GPUs entreguem alta transferência e processamento de dados, há custos associados: latência de transferência de dados entre CPU e GPU, custos de gerenciamento e aumento de complexidade de programação.
