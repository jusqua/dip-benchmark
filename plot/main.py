...
def parse_benchmark_data():
    csv_path = Path(__file__).parent.parent / "assets" / "results.csv"
    ...
    for operation in operation_columns:
        data[operation] = [row / 10000 * 1000000 for row in df[operation]]
...
def create_horizontal_bar_plot(operations, frameworks, data, title, filename, color_palette=None):
    ...
    ax.set_xlabel('Time (microseconds per operation)', fontsize=12)
    ...
    output_path = Path(__file__).parent.parent / "assets" / filename
...
def main():
    ...
    conv3x3_ops = ['Convolution (3x3 Gaussian Blur)', 'Convolution (1x3+3x1 Gaussian Blur)', 'Gaussian Blur (3x3)']
    create_horizontal_bar_plot(
    ...
    conv5x5_ops = ['Convolution (5x5 Gaussian Blur)', 'Convolution (1x5+5x1 Gaussian Blur)']
    create_horizontal_bar_plot(
